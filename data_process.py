...
PLATEAU_SLOPE_THR = 0.08  # 平台内部斜率阈值（单位/点）
PLATEAU_EDGE_JUMP_K = 1.5  # 边界跳变需超过幅值比例
DRIFT_ABS_MIN = 5.0  # 漂移绝对幅值最小门槛
PLATEAU_MIN_AMPLITUDE = 15.0  # 平台型偏移最小幅值
PLATEAU_FLATNESS_THR = 0.60  # 平顶比例阈值（越高越平）
PLATEAU_EDGE_RATIO_THR = 1.8  # 边缘突变强度/中部变化强度阈值
...
def classify_drift_segments(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    drift_dev = (drift - drift.rolling(288, center=True, min_periods=24).median()).abs()
    thr = max(DRIFT_ABS_MIN, DRIFT_SEGMENT_MAD_K * 1.4826 * robust_mad(drift))
    cand = (drift.abs() > thr) | (drift_dev > thr * 0.6)
    ...
    for s, e in segs:
        ...
        pre = out["relative_slow_trend"].iloc[max(0, s - EDGE_WINDOW):s]
        post = out["relative_slow_trend"].iloc[e + 1:min(len(out), e + 1 + EDGE_WINDOW)]
        pre_med = pre.median() if pre.notna().sum() else 0.0
        post_med = post.median() if post.notna().sum() else 0.0
        seg_med = seg["relative_slow_trend"].median()

        bg = np.nanmedian([pre_med, post_med])
        amp = float(seg_med - bg)
        left_jump = float(seg["relative_slow_trend"].iloc[0] - pre_med) if len(seg) else 0.0
        right_jump = float(post_med - seg["relative_slow_trend"].iloc[-1]) if len(seg) and post.notna().sum() else 0.0

        y = seg["relative_slow_trend"].fillna(seg_med).to_numpy()
        x = np.arange(len(y))
        ...
        seg_score = 0.5 * flatness_ratio + 0.5 * min(edge_sharpness_ratio / 3.0, 1.0)
        ...
        gradual_like = (
            n >= GRADUAL_MIN_LEN
            and abs(slope) >= GRADUAL_SLOPE_MIN
            and (flatness_ratio < PLATEAU_FLATNESS_THR or edge_sharpness_ratio < PLATEAU_EDGE_RATIO_THR)
        )
        ...
        if STUCK_SUPPRESS_DRIFT and stuck_ratio > 0.6:
            ...
            seg_type = "gradual_drift"
        else:
            seg_type = "gradual_drift"

        if plateau_like:
            out.loc[out.index[s:e + 1], "level_shift"] = out["relative_slow_trend"].iloc[s:e + 1].fillna(0.0).to_numpy()
            out.loc[out.index[s:e + 1], "is_level_shift"] = True
            out.loc[out.index[s:e + 1], "drift_segment_type"] = "level_shift"
        elif seg_type == "gradual_drift":
            out.loc[out.index[s:e + 1], "gradual_drift"] = out["relative_slow_trend"].iloc[s:e + 1].fillna(0.0).to_numpy()
            out.loc[out.index[s:e + 1], "is_gradual_drift"] = True
            out.loc[out.index[s:e + 1], "drift_segment_type"] = "gradual_drift"
        else:
        ...
        is_fault = (seg_type == "gradual_drift") and (status_ratio >= 0.2)
    ...
    # 仅对明确gradual段赋值，不让gradual吞掉全部慢变
...
def build_cleaned_series(df: pd.DataFrame) -> pd.DataFrame:
    ...
    analysis = obs.copy()
    analysis = analysis - out["gradual_drift"].fillna(0.0)
